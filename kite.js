// Call the kiteMob() function with your script at an interval, and edit the mtypes below. 
...
var drawDebug = true;
    ...
    for(id in parent.entities)
    {
        ...
        var nearestT = get_nearest_monster(); 
        if(entity.mtype == mob && entity == nearestT )			
        {   
            goal = {x: entity.real_x, y: entity.real_y};
		   if(can_use(attack) && in_attack_range(entity)){
			attack(entity);
		   }
			if(can_use("scare",entity) && in_attack_range(entity)){	
		  use_skill("scare",entity);
		}
			
		if(can_use("3shot",entity) && in_attack_range(entity) && character.mp > 400){	
		    use_skill("3shot",entity);
		}
		else {
            break;
		}
...
var avoidTypes = (mob)                                             // Change this to the mob to kite
